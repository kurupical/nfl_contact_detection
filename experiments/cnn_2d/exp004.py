...
class ThreeLayerConv1DUnit(nn.Module):
    def __init__(self,
        ...
        self.fc = nn.Conv1d(config.n_frames, config.hidden_size_1d, 13, bias=False, stride=2)
        self.bn = nn.BatchNorm1d(config.hidden_size_1d)
        self.do = nn.Dropout(config.dropout_seq)
        self.fc2 = nn.Conv1d(config.hidden_size_1d, config.hidden_size_1d * 2, 7, bias=False, stride=2)
        self.bn2 = nn.BatchNorm1d(config.hidden_size_1d * 2)
        self.do2 = nn.Dropout(config.dropout_seq)
        self.fc3 = nn.Conv1d(config.hidden_size_1d * 2, config.hidden_size_1d * 2, 5, bias=False, stride=1)
        self.bn3 = nn.BatchNorm1d(config.hidden_size_1d * 2)
...
class Model(nn.Module):
    def __init__(self,
        ...
        if config.seq_model == "1dcnn_3layers":
            self.fc = nn.Linear(self.seq_model.model.num_features, config.n_predict_frames)
        else:
            self.fc = nn.Linear(config.n_frames, config.n_predict_frames)
...
if __name__ == "__main__":
    ...
    image_path = "images_96x96_v2"
    exp_name = f"2d_1dcnn_simple"
    config = Config(exp_name=exp_name, n_predict_frames=1, n_frames=31, seq_model="1dcnn")
    main(config)
    ...
    for dropout_seq in [0, 0.1, 0.5]:
        image_path = "images_96x96_v2"
        exp_name = f"2d_1dcnn_3layers_dropout_seq{dropout_seq}"
        config = Config(exp_name=exp_name, n_predict_frames=1, n_frames=31, seq_model="1dcnn_3layers", dropout_seq=dropout_seq)
        main(config)

    for model_2d_name in ["tf_efficientnet_b1_ns", "tf_efficientnet_b2_ns"]:
        image_path = "images_96x96_v2"
        exp_name = f"2d_cnn2d={model_2d_name}"
        config = Config(exp_name=exp_name, n_predict_frames=1, n_frames=31, seq_model="1dcnn_3layers",
                        model_name_2d=model_2d_name)
        main(config)

    for n_predict_frames in [3, 5]:
        image_path = "images_96x96_v2"
        exp_name = f"2d_n_predict_frames{n_predict_frames}"
        config = Config(exp_name=exp_name, n_predict_frames=n_predict_frames)
        main(config)

    image_path = "images_96x96_v2"
    exp_name = f"2d_n_frames63_step1"
    config = Config(exp_name=exp_name, n_frames=63, step=1)
    main(config)