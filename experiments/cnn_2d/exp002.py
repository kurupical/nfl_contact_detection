...
class NFLDataset(Dataset):
    ...
    def _get_item_information(self, logger: Logger):
        ...
        for key, w_df in tqdm.tqdm(self.df.groupby(["game_play", "view", "nfl_player_id_1", "nfl_player_id_2"])):
            game_play = key[0]
            view = key[1]
            id_1 = key[2]
            id_2 = key[3]
            ...
            for i in range(0, len(w_df) + config.interval, config.interval):
                max_frame_idx = min(
                    i + self.config.n_frames * self.config.step,
                    len(w_df) - 1
                )
                min_frame_idx = max_frame_idx - self.config.n_frames * self.config.step
                if min_frame_idx < 0:
                    continue
                indice = np.arange(min_frame_idx, max_frame_idx, self.config.step)
                center_idx = min(
                    i + (self.config.n_frames // 2) * self.config.step,
                    len(w_df) - 1 - (self.config.n_frames // 2) * self.config.step
                )
                predict_frames_indice = np.arange(
                    center_idx - (self.config.n_predict_frames // 2) * self.config.step,
                    center_idx + (self.config.n_predict_frames // 2) * self.config.step + 1,
                    self.config.step
                )
        ...
        logger.info(f"finished. extracted={len(self.items)} (total={len(self.df)}, failed: {failed_count})")
...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        center_idx = self.config.n_frames // 2
        predict_frames_indice = np.arange(
            center_idx - self.config.n_predict_frames // 2,
            center_idx + self.config.n_predict_frames // 2 + 1
        )
        x = x[:, predict_frames_indice, :]
        return x
