...
def main(config):
    try:
        ...
        if config.debug:
            num_workers = 0
        elif type(config) == ConfigForGNN in config.model_name:
            num_workers = 0
        elif "cnn_2d_" in config.model_name and config.n_frames == 1:
            num_workers = 2
...
if __name__ == "__main__":
    ...
    # main(config)
    #
    # exp_name = "transformer_log1pFalse"
    # config = ConfigForTransformer(exp_name=exp_name, apply_log1p=False)
    # main(config)
    #
    # exp_name = "transformer_apply_ffnFalse"
    # config = ConfigForTransformer(exp_name=exp_name, apply_ffn=False)
    # main(config)
    #
    # for hidden_size_transformer in [256, 512]:
    #     exp_name = f"transformer_hid{hidden_size_transformer}"
    #     config = ConfigForTransformer(exp_name=exp_name, hidden_size_transformer=hidden_size_transformer)
    #     main(config)

    # for hidden_size_transformer in [128, 512]:
    #     exp_name = f"transformer_hid{hidden_size_transformer}_epochs5_linear"
    #     config = ConfigForTransformer(exp_name=exp_name, hidden_size_transformer=hidden_size_transformer, epochs=5, warmup_ratio=0.1,
    #                                   scheduler="linear")
    #     main(config)
    # for num_layer_transformer in [1, 2]:
    #     exp_name = f"transformer_num_layer_transformer{num_layer_transformer}"
    #     config = ConfigForTransformer(exp_name=exp_name, num_layer_transformer=num_layer_transformer)
    #     main(config)
    #
    # for model in [
    #     "legacy_seresnet18", "legacy_seresnet34",
    #     "efficientnet_es_pruned", "eca_nfnet_l0",
    #     "resnetv2_50d_evos", "legacy_seresnet50",
    # ]:
    ...
    #     main(config)
    #
    # exp_name = f"2.5d3d_v10_baseline(sep_is_g=True, gamma_aug=False)"
    ...
    #     exp_name = f"2.5d3d_n_predict_frames{n_predict_frames}"
    #     config = Config(exp_name=exp_name, n_frames=87, seq_model="3dcnn_simple",
    #                     model_name=f"cnn_2.5d3d_legacy_seresnet18", epochs=2, step=3,
    #                     step_size_ratio=0.6,
    #                     hidden_size_3d=512,
    ...
    # for hidden_size_transformer in [128, 512]:
    #     exp_name = f"transformer_hid{hidden_size_transformer}_epochs5_linear"
    #     config = ConfigForTransformer(exp_name=exp_name, hidden_size_transformer=hidden_size_transformer, epochs=5,
    #                                   warmup_ratio=0.1)
    #     main(config)
    #
    # for warmup_ratio in [0.01, 0.05]:
    #     exp_name = f"transformer_hid512_warmup{warmup_ratio}"
    #     config = ConfigForTransformer(exp_name=exp_name, hidden_size_transformer=hidden_size_transformer, epochs=5,
    #                                   warmup_ratio=warmup_ratio)
    #     main(config)
    #
    # for nhead in [16, 32]:
    #     exp_name = f"transformer_hid512_head{nhead}_epochs5_linear"
    #     config = ConfigForTransformer(exp_name=exp_name, hidden_size_transformer=512, epochs=5, warmup_ratio=0.1,
    #                                   nhead=nhead)
    #     main(config)
    #
    # for epochs in [3, 5, 7]:
    #     exp_name = f"transformer_epochs{epochs}_linear"
    #     config = ConfigForTransformer(exp_name=exp_name, hidden_size_transformer=128, epochs=epochs, warmup_ratio=0.05)
    #     main(config)
    #
    ...
    # main(config)
    #
    # exp_name = f"2d1d_contact0.2_resnet34"
    ...
    #                 interpolate_image=False,
    #                 calc_single_view_loss_weight=1,
    #                 image_feature_dir="../../output/cnn_3d/exp042/20230129134256_2d_pretrain_legacy_seresnet34"
    #                 )
    # main(config)

    for epochs in [5]:
        exp_name = f"transformer_5epochs_fc1layernorm1"
        config = ConfigForTransformer(exp_name=exp_name, hidden_size_transformer=128, epochs=epochs, warmup_ratio=0.05)
        main(config)
