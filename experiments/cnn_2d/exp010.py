...
class SequenceModel(nn.Module):
    def __init__(self,
        ...
        if config.seq_model == "1dcnn":
            self.model = nn.Conv1d(
                in_channels=self.n_frames,
                out_channels=config.n_predict_frames,
...
class Model(nn.Module):
    def __init__(self,
        ...
        super().__init__()
        self.config = config
        self.cnn_2d = timm.create_model(
            model_name=config.model_name_2d,
            checkpoint_path=config.model_checkpoint_path,
            num_classes=1
        )
        self.cnn_2d.classifier = nn.Identity()
        if config.freeze:
            for param in self.cnn_2d.parameters():
    ...
    def forward(self, x):
        ...
        x = self.seq_model(x)  # (bs, seq_len, features) -> (bs, n_predict_frames, features)
        x = self.fc(x)  # (bs, n_predict_frames, features) -> (bs, n_predict_frames, 1)
        x = x.squeeze(2)
        return x
...
if __name__ == "__main__":
    ...
    for n_frames in [31, 63, 127]:
        image_path = "images_96x96_v2"
        exp_name = f"2d_1dcnn_simple_pretrained_eff0"
        config = Config(exp_name=exp_name, n_predict_frames=3, n_frames=n_frames, seq_model="1dcnn",
                        model_name_2d="tf_efficientnet_b0_ns",
                        model_checkpoint_path="../../output/cnn_3d/exp001/20221227064128_2d_pretrained_tf_efficientnet_b0_ns/epoch0.pth")
        main(config)