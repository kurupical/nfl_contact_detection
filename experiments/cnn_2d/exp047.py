...
class Model2DTo1D(nn.Module):
    def __init__(self,
        ...
        if self.config.fc == "simple":
            self.fc = nn.LazyLinear(config.n_predict_frames, bias=False)
        elif self.config.fc == "2layer":
            self.fc = nn.Sequential(
                nn.LazyLinear(32, bias=False),
                nn.Dropout(self.config.fc_dropout),
                nn.GELU(),
                nn.Linear(32, config.n_predict_frames, bias=False)
            )
        self.fc_weight = nn.LazyLinear(1, bias=False)
        self.fc_contact = nn.LazyLinear(config.n_predict_frames, bias=False)
        self.fc_g = nn.LazyLinear(config.n_predict_frames, bias=False)
        self.fc_endzone_contact = nn.LazyLinear(config.n_predict_frames, bias=False)
        self.fc_sideline_contact = nn.LazyLinear(config.n_predict_frames, bias=False)
        self.fc_endzone_g = nn.LazyLinear(config.n_predict_frames, bias=False)
        self.fc_sideline_g = nn.LazyLinear(config.n_predict_frames, bias=False)
...
class Model2p5DTo3D(nn.Module):
    def __init__(self,
        ...
        if self.config.feature_window > 0:
            self.fc_feature = nn.LazyLinear(self.config.feature_hidden_size, bias=False)
            transformer_encoder = nn.TransformerEncoderLayer(
...
if __name__ == "__main__":
    exp_name = f"2.5d3d_v25_fcbiasFalse"
    ...
    main(config)
    exp_name = f"2.5d3d_v25_1epochs_for_pretrain_3dcnn"
    config = Config(exp_name=exp_name, n_frames=87, seq_model="3dcnn_simple",
                    use_data_step=1,
                    model_name=f"cnn_2.5d3d_legacy_seresnet18", epochs=2, step=3,
                    step_size_ratio=1.5,
