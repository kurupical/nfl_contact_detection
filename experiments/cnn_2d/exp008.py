    ...
    def _get_item_information(self, df: pd.DataFrame, logger: Logger):
        ...
        for key, w_df in tqdm.tqdm(df.groupby(["game_play", "view", "nfl_player_id_1", "nfl_player_id_2"])):
            ...
            for idx, i in enumerate(rank_1_indices):
                ...
                if distances[idx] > 400 and contacts[idx] == 0 and np.random.random() > self.config.negative_sample_ratio/0.5 and not self.test:
                    continue
                if distances[idx] > 200 and contacts[idx] == 0 and np.random.random() > self.config.negative_sample_ratio/0.5 and not self.test:
                    continue
                if contacts[idx] == 0 and np.random.random() > self.config.negative_sample_ratio and not self.test:
                    continue
...
#         x = self.cnn_2d.forward_features(x)  # (bs*seq_len, features)
...
class Model3D(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 2, 1, 3, 4)  # (bs, seq_len, C, W, H)
        x = x.reshape(bs*seq_len, C, W, H)  # (bs*seq_len, C, W, H)
        x = self.cnn_2d.forward_features(x)  # (bs*seq_len, C', W', H')
        _, C_, W_, H_ = x.shape
        x = x.reshape(bs, seq_len, C_, W_, H_)  # (bs, seq_len, C_, W_, H_)
        x = x.permute(0, 2, 1, 3, 4) # (bs, C_, seq_len, W_, H_)

        x = self.seq_model(x)  # (bs, seq_len, features)
        x = F.adaptive_avg_pool3d(x, 1).squeeze(4).squeeze(3).squeeze(2)  # (bs, C)
        x = self.fc(x)  # (bs, seq_len, n_predict_frames)
        return x
...
if __name__ == "__main__":
    ...
    exp_name = f"2d_3dcnn_simple_sampling"
    config = Config(exp_name=exp_name, n_predict_frames=1, n_frames=31, seq_model="3dcnn_simple")
    main(config)
