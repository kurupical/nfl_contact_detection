...
class Config:
    ...
    base_dir: str = "../../output/preprocess/images"
    image_path: str = "images_128x96_v9"
    data_dir: str = f"../../output/preprocess/master_data_v3"
...
class NFLDatasetForPretrainedFeature(NFLDataset):
    def __init__(self,
                 df: pd.DataFrame,
                 base_dir: str,
                 logger: Logger,
                 config: Config,
                 test: bool,
                 use_filelist: bool = False,
                 submission_mode: bool = True,
                 image_dict: dict = None):
        super().__init__(df, base_dir, logger, config, test,
                         use_filelist=use_filelist, submission_mode=submission_mode, image_dict=image_dict)
        self.img_shape = list(self.image_dict.values())[0]

        image_features = np.load(f"{self.config.image_feature_dir}/preds.npy")
        fnames = np.load(f"{self.config.image_feature_dir}/files.npy")
        self.image_dict = {}
...
class Model2DTo1D(nn.Module):
    ...
    def forward(self, x, is_g, feature):
        bs, seq_len, F = x.shape  # shape = (bs, seq_len*n_view, features)
        x = x.reshape(bs*2, seq_len//2, F)  # shape = (bs*n_view, seq_len, features)

        x = self.seq_model(x).mean(dim=2)  # (bs*n_view, seq_len, features) -> (bs*n_view, hidden)
...
def main(config):
    try:
        ...
        elif "cnn_2d_" in config.model_name:
            logger.info("set n_frames = 1")
            config.n_frames = 1
...
if __name__ == "__main__":
    #
    # for batch_size in [8, 128, 512]:
    #     exp_name = f"gcn_batch_size{batch_size}"
    #     config = ConfigForGNN(
    #         exp_name=exp_name,
    #         model_name=f"gnn_egat",
    #         epochs=5,
    #         batch_size=batch_size,
    #     )
    #     main(config)
    #
    # exp_name = f"gcn_scheduler_linear"
    # config = ConfigForGNN(
    #     exp_name=exp_name,
    #     model_name=f"gnn_egat",
    #     batch_size=128,
    ...
    for model in [
        ...
        config = Config(exp_name=exp_name, seq_model="3dcnn_simple",
                        model_name=f"cnn_2d_{model}", epochs=1, step=3,
                        negative_sample_ratio_g=1,
                        negative_sample_ratio_far=1,
                        negative_sample_ratio_close=1,
                        hidden_size_3d=512,
    ...
    #
    # for model in [
    #     "legacy_seresnet18", "resnet18", "mobilevitv2_050", "mixer_l16_224", "legacy_seresnet34",
    #     "deit_tiny_distilled_patch16_224", "mobilevit_xs", "regnety_006", "tf_efficientnet_lite0",
    #     "efficientnet_es_pruned", "tf_efficientnet_es", "tf_efficientnet_lite1", "tf_efficientnet_b0",
    #     "tf_efficientnet_b1"
    # ]:
    #     exp_name = f"2d_pretrain_{model}"
    #     config = Config(exp_name=exp_name, seq_model="3dcnn_simple",
