...
class Model:
    ...
    def feature_engineering(self,
        ...
        for window in tqdm.tqdm([5, 15], desc="rolling feature"):
            for lag in [0, 1, 5, 10, -1, -5, 10]:
                for col in ["x_position", "y_position", "speed", "Endzone_x", "Endzone_y", "Sideline_x", "Sideline_y"]:
                    col_name = f"corr_{col}_window{window}_lag{lag}"
                    if lag == 0:
                        col1 = f"{col}_1"
                        col2 = f"{col}_2"
                    else:
                        col1 = f"{col}_1_diff{lag}"
                        col2 = f"{col}_2_diff{lag}"
                    df[col_name] = df[col1].rolling(window=window).corr(df[col2])
        agg_cols = [
            "distance",
            "Endzone_move_helmet_1", "Endzone_move_helmet_2",
            "Sideline_move_helmet_1", "Sideline_move_helmet_2",
            ...
            "Endzone_distance_helmet_lag-1",
            "Sideline_distance_helmet_lag-1",
            "Endzone_distance_helmet_lag-5",
            "Sideline_distance_helmet_lag-5",
        ...
        agg_col3 = [
            ...
            "Endzone_move_helmet_1_lag-1",
            "Sideline_move_helmet_1_lag-1",
            "Endzone_move_helmet_1_lag-5",
            "Sideline_move_helmet_1_lag-5",
            "distance_1_lag1",
            "distance_1_lag5",
            "distance_1_lag10",
            "distance_1_lag-1",
        ...
        for groupby_col in ["is_g", "n_player_distance_all_in_3", "step", "is_same_team",
                            "n_player_Sideline_distance_helmet_all_in_2000", "n_player_Sideline_distance_helmet_all_in_1000",
...
def main():
    ...
    model.train(df, use_half_data=True, apply_focal_loss=False, fold=0, key="game_play")
    del model.logger
    #
    # for depth in [8]:
    #     output_dir = f"../../output/lgbm/{os.path.basename(__file__).replace('.py', '')}/{dt.now().strftime('%Y%m%d%H%M%S')}"
