...
def matthews_corrcoef_(y_pred, data):
    score = matthews_corrcoef(data.get_label(), y_pred > 0.3)
    return 'matthews', score, True
...
def main():
    ...
    model = LGBMModel(output_dir=output_dir, logger=logger, exp_name="exp006_bugfix", debug=debug, fast_mode=True,
                      params=params)
    model.train(df, use_half_data=True, fold=2)
    del model.logger
