...
class Model:
    ...
    def feature_engineering(self,
        ...
        df = df.sort_values(["game_play", "nfl_player_id_1", "nfl_player_id_2"]).reset_index(drop=True)
        for col in ["x_position", "y_position", "distance"]:
            for player_id in [1, 2]:
                col_name = f"{col}_{player_id}_diff1"
                df[f"{col_name}_abs"] = df[col_name].abs()
                for window in [5, 15]:
                    df[f"{col_name}_{window}_std"] = df.groupby(["game_play", "nfl_player_id_1", "nfl_player_id_2"])[col_name].rolling(window=window).std().values
                    df[f"{col_name}_{window}_absmean"] = df.groupby(["game_play", "nfl_player_id_1", "nfl_player_id_2"])[f"{col_name}_abs"].rolling(window=window).mean().values
                    df[f"div_{col_name}_{window}_std_absmean"] = df[f"{col_name}_{window}_absmean"] / df[f"{col_name}_{window}_std"]
        ...
        agg_cols = [
            ...
            "distance_lag1", "distance_lag5", "distance_lag20",
            "distance_diff1", "distance_diff5", "distance_diff20",
...
def main():
    ...
    # model.train(df, use_half_data=True, apply_focal_loss=False, fold=0, key="game_key")
    ...
    model = Model(output_dir=output_dir, logger=logger, exp_name="exp006_bugfix", debug=debug, fast_mode=True,
                  params=params,model_name="catboost")
