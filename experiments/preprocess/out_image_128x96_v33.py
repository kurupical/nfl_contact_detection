...
PLAYER = (255, 0, 0)
output_size = (128, 96)
output_dir = f"../../../work/images_{output_size[0]}x{output_size[1]}_v33"
traintest = "train"
...
def main():
    ...
    for key, df_ in tqdm.tqdm(df_helmets_.groupby(["game_play", "view", "nfl_player_id", "team"])):
        ...
        for col in ["x", "y", "width", "height"]:
            df_[f"{col}_interpolate"] = df_[col].interpolate(limit=10, limit_area="inside")
    ...
    for i, game_play in enumerate(tqdm.tqdm(game_plays)):
        ...
        nfl_player_id_2s = gp["nfl_player_id_2"].values
        for col in ["x", "y", "width", "height"]:
            mean = gp[[f"{col}_1", f"{col}_2"]].mean(axis=1)
            mean_interpolate = gp[[f"{col}_interpolate_1", f"{col}_interpolate_2"]].mean(axis=1)

            gp[col] = [mean[i] if nfl_player_id_2s[i] == "G" else mean_interpolate[i] for i in range(len(gp))]
