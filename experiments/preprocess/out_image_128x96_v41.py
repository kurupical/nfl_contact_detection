...
PLAYER = (255, 0, 0)
output_size = (128, 96)
output_dir = f"../../../work/images_{output_size[0]}x{output_size[1]}_v41"
traintest = "train"
...
def main():
    ...
    for i, game_play in enumerate(tqdm.tqdm(game_plays)):
        ...
        for col in ["width", "height"]:
            gp[col] = gp[[f"{col}_1", f"{col}_2"]].max(axis=1)
        gp["bbox_size"] = gp[["width", "height"]].mean(axis=1)
        gp["bbox_size"] = gp["bbox_size"].rolling(11, min_periods=1, center=True).mean()
        for view in ["Endzone", "Sideline"]:
            ...
            for frame in range(frame_count):
                ...
                img_ = cv2.addWeighted(src1=img_, alpha=0.1, src2=img_filter, beta=0.9, gamma=0)
                for key, w_df in data_dict.items():
                    ...
                    img = cv2.addWeighted(src1=img, alpha=0.1, src2=img_filter, beta=0.9, gamma=0)
