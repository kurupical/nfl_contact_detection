...
PLAYER = (255, 128, 128)
output_size = (128, 96)
output_dir = f"../../../work/images_{output_size[0]}x{output_size[1]}_v34"
traintest = "train"
    ...
    for i, game_play in enumerate(tqdm.tqdm(game_plays)):
        ...
        for view in ["Endzone", "Sideline"]:
            ...
            for frame in range(frame_count):
                it_worked, img_original = vidcap.read()

                if frame not in frames:
                    continue

                img_ = np.zeros(img_original.shape) + 128
                ...
                for key, w_df in data_dict.items():
                    ...
                    img = img_original[down:top, left:right]

                    img = cv2.resize(img, dsize=output_size)
                    out_fname = f"{output_dir}/{game_play}/{view}/{key[0]}_{key[1]}_{frame}_original.jpg"
                    os.makedirs(os.path.dirname(out_fname), exist_ok=True)
                    cv2.imwrite(out_fname, img)

                    img = img_[down:top, left:right]
                    img_filter = img.copy()
                    ...
                    img = cv2.resize(img_filter, dsize=output_size)
                    out_fname = f"{output_dir}/{game_play}/{view}/{key[0]}_{key[1]}_{frame}_filter.jpg"
                    os.makedirs(os.path.dirname(out_fname), exist_ok=True)
                    cv2.imwrite(out_fname, img)
