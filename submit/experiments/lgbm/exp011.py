...
def main():
    ...
    for _ in range(1000):
        ...
        params = {
            'objective': 'binary',
            'metrics': 'auc',
            'num_leaves': np.random.choice([16, 32, 64, 128, 256, 512]),
            'max_depth': -1,
            'bagging_fraction': np.random.choice([0.1, 0.3, 0.5, 0.7, 0.9]),  # 0.5,
            'feature_fraction': np.random.choice([0.1, 0.3, 0.5, 0.7, 0.9]),
            'bagging_seed': 0,
            'reg_alpha': np.random.choice([3, 5, 10, 20, 50]),
            'reg_lambda': np.random.choice([3, 5, 10, 20, 50]),
            'min_data_in_leaf': np.random.choice([1, 10, 50, 100, 500, 1000, 5000, 10000]),
            'random_state': 0,
            'verbosity': -1,
            "min_gain_to_split": np.random.choice([0, 0.1, 0.5, 1, 3]),
            "n_estimators": 20000,
            "early_stopping_rounds": 100,
            "learning_rate": 0.1,
            "extra_trees": np.random.choice([False, False, True]),
            "n_jobs": 32
        }
        model = LGBMModel(output_dir=output_dir, logger=logger, exp_name="exp006_bugfix", debug=debug, fast_mode=True,
                          params=params, use_features=use_features)
